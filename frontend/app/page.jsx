...
const MedicalMonitoringDashboard = memo(function MedicalMonitoringDashboard() {
  ...
  // Update vitals from WebSocket
  useEffect(() => {
    if (lastMessage && lastMessage.type === 'vitals_update' && lastMessage.data && lastMessage.data.length > 0) {
      // Use the most recent data point
      const latestData = lastMessage.data[lastMessage.data.length - 1];
      if (latestData.heart_rate) setHeartRate(latestData.heart_rate);
      if (latestData.respiration) setRespirationRate(latestData.respiration);
    }
  }, [lastMessage]);
  ...
  // ONLY used if no WebSocket data is available (fallback/demo mode)
  const updateVitalSigns = useCallback(() => {
    // Only update if we haven't received data recently (simple check could be added here)
    // For now, we'll leave this as a fallback or remove it if we want pure real-time
    // But to keep the "alive" feel when no data is flowing, we might keep it or disable it.
    // Let's disable it if we want to rely solely on WS, but the prompt asked to connect data.
    // If I remove it, the dashboard will be static until data comes in.
    // Let's comment it out to prove the connection works.
    /*
    setHeartRate(prev => {
