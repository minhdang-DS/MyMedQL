...
export default function PatientLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await loginPatient(formData.patientId, formData.password);
      // Redirect to patient dashboard after successful login
      router.push(`/roles/patient/dashboard`);
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl border p-8" style={{ borderColor: palette.border }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-lg border transition-all focus:outline-none focus:ring-2 ${errors.patientId ? "border-red-300 focus:ring-red-200" : "focus:ring-blue-200"
                  }`}
                style={{
          ...
          <div className="mt-6 pt-6 border-t text-center" style={{ borderColor: palette.border }}>
            <p className="text-xs" style={{ color: palette.textLight }}>
              Need help? Contact <Link href="#" className="font-medium hover:underline" style={{ color: palette.primary }}>Support</Link>
            </p>
          </div>
        </div>
      </div>
