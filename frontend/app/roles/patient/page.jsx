    ...
    <div className="min-h-screen" style={{ backgroundColor: palette.lightBg, color: palette.navy }}>
      <div className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold" style={{ color: palette.primaryBright }}>Patient view</p>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-3">
          <div className="rounded-2xl border bg-white p-6 shadow-sm md:col-span-2" style={{ borderColor: palette.greyBorder }}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {vitals.map((v) => (
                <div key={v.label} className="rounded-xl border bg-white px-4 py-3 shadow-sm" style={{ borderColor: palette.greyBorder }}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-semibold" style={{ color: palette.navy }}>{v.label}</span>
                    <span className="text-xs" style={{ color: palette.primaryBright }}>{v.trend}</span>
                  </div>
          ...
          <div className="rounded-2xl border bg-white p-6 shadow-sm" style={{ borderColor: palette.greyBorder }}>
            ...
            <div className="mt-4 space-y-3 text-sm">
              {alerts.map((a) => (
                <div key={a.title} className="rounded-lg border bg-[#F8FAFC] px-3 py-2 shadow-sm" style={{ borderColor: palette.greyBorder }}>
                  <div className="flex items-center justify-between">
                    <span className="font-semibold" style={{ color: palette.navy }}>{a.title}</span>
                    <span className={`text-xs font-semibold ${a.status === 'Resolved' ? '' : ''}`} style={a.status === 'Resolved' ? { color: palette.success } : { color: palette.primary }}>
                      {a.status}
                    </span>
                  </div>
            ...
            <button className="mt-4 w-full rounded-full px-4 py-2 text-xs font-semibold text-white shadow" style={{ backgroundColor: palette.primary }}>View full history</button>
          </div>
        </div>
